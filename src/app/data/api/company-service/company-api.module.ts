...
@NgModule({
  ...
  exports: [],
  declarations: [],
  providers: [PositionApiService, DepartmentApiService, ApiConfiguration],
})
export class CompanyApiModule {
  static forRoot(
    params: ApiConfigurationParams
  ): ModuleWithProviders<CompanyApiModule> {
    return {
  ...
  constructor(
    ...
    if (parentModule) {
      throw new Error(
        'CompanyApiModule is already loaded. Import in your base AppModule only.'
      );
    }
    if (!http) {
      throw new Error(
