...
export class AvatarApiService extends BaseService {
  ...
  getAvatar(params: {
    userId: string;
  }): Observable<OperationResultResponseImagesResponse> {

    return this.getAvatar$Response(params).pipe(
      map((r: StrictHttpResponse<OperationResultResponseImagesResponse>) => r.body as OperationResultResponseImagesResponse)
    );
  }
