...
export class NewsApiService extends BaseService {
	 ...
	 */
	createNews$Response(params: { body: CreateNewsRequest }): Observable<StrictHttpResponse<OperationResultResponse>> {
	 ...
	 * This method sends `application/json` and handles request body of type `application/json`.
	 */
	createNews(params: { body: CreateNewsRequest }): Observable<OperationResultResponse> {
	...
	getNews(params: {
		...
		return this.getNews$Response(params).pipe(
			map(
				(r: StrictHttpResponse<OperationResultResponseNewsResponse>) =>
					r.body as OperationResultResponseNewsResponse
			)
		);
	}
